...
Install custom utility function package
```{r}
...
Define function to load in 10X sequencing data
```{r}
init_seurat <- function(data.dir, proj.name, min.cells = 3, min.features = 200){
  print(data.dir)
  print(proj.name)
...
```{r}
base.dir <- "all_aligned_samples/organized"
sample.list <- list.dirs(base.dir, recursive = FALSE, full.names = FALSE)
...
for (sample in sample.list){
  for (cell.type in cell.types){
    cell.ranger.v2.folder = paste(base.dir, "/", sample, cell.type, 
                                  "/outs/filtered_gene_bc_matrices/GRCh38", sep = "")
    cell.ranger.v3.folder = paste(base.dir, "/", sample, cell.type, 
...
seuratObjs = list()
seuratObjs = mapply(FUN = init_seurat, data.dir = data.sources.table$data.source,
...
sample.metadata.pbmc %>% 
  mutate(Group = factor(Group, levels = c("Control", "Carrier", "Proband"))) %>%
  ...
  stat_compare_means(comparisons = 
                       list(c("Control", "Carrier"), c("Carrier", "Proband"), c("Control", "Proband")), 
                     method = "t.test", 
                     label = "p.signif", 
...
merged_subset@meta.data %>% 
	...
	scale_colour_gradient(low = "steelblue", high = "salmon", name = "% mito") +
  	...
  	scale_y_log10() + 
  	theme_classic() +
  	geom_vline(xintercept = 500, linetype = "dashed", color = "springgreen4") +
  	geom_hline(yintercept = 250, linetype = "dashed", color = "springgreen4") +
...
merge(data.frame(pcs$x) %>% rownames_to_column("smpl"), sample.metadata.pbmc, by = "smpl") %>% mutate(smpl_type = str_split(smpl, "_", simplify = TRUE)[, 1]) %>%
  ...
  scale_color_manual(values = c("steelblue", "springgreen4", "salmon"), breaks = c("control", "carrier", "proband"), name = "Sample Type", 
                     labels = c("Control", "Carrier", "Proband")) +
  ...
  theme(axis.title.x = element_text(size = 10),
        ...
        plot.title = element_text(hjust = 0.5, size = 14), legend.text = element_text(size = 10))
```
...
png(filename = str_c(output.dir, "heatmap_corr_pbmc.png"), height = 8, width = 9, units = "in", res = 1200)
Heatmap(cor_cross_smpl, name = "Pearson r", col = col_fun)
dev.off()
```
...
mean(cor_cross_smpl_mono)
png(filename = str_c(output.dir, "heatmap_corr_mono.png"), height = 8, width = 9, units = "in", res = 1200)
...
merge(data.frame(pcs$x) %>% rownames_to_column("smpl"), sample.metadata.pbmc, by = "smpl") %>% mutate(smpl_type = str_split(smpl, "_", simplify = TRUE)[, 1]) %>%
  ...
  geom_point(size = 3) + 
  scale_color_manual(values = c("#BCAF9C", "#773344"), breaks = c("nih", "nwh"), labels = c("NIH", "Northwell Health"), name = "Sample Source")+
  scale_shape_manual(values = c(17, 19), breaks = c("new", "old"), labels = c("2021", "2019/2020"),
                     name = "Batch") + theme_classic() + xlab("PC 1") + ylab("PC 2") + 
  ggtitle("PBMC samples") + 
  theme(axis.title.x = element_text(size = 10),
...
save.image(str_c(output.dir, "pbmc_int_workspace_02272022_bckp.RDS"))
...
save.image(str_c(output.dir, "post_int_workspace_02272022_bckp.RDS"))
```
...
umap_metadata_total[sample(1:nrow(umap_metadata_total)), ] %>% 
  ...
  theme(axis.title.x = element_text(size = 12),
        ...
        legend.text = element_text(size = 12)) + guides(colour = guide_legend(override.aes = list(size=6))) + 
...
tmp <- data.frame(table(pbmc.combined$smpl, Idents(pbmc.combined))) %>% 
  ...
  mutate(smpl_type = factor(smpl_type, levels = c("Control", "Carrier", "Proband")))
...
_seurat_obj_ Input Seurat object
_cluster_ Cluster to be used to subset Seurat object for DE analysis
_slot_ Which slot to use data from (by default - "counts", since DESeq2 expects raw, untransformed counts)
_assay_ Which assay to use data from (by default - "RNA", since DESeq2 expects unintegrated counts)

Output

List object with DESeqDataSet ( _dds_ ), vst-transformed object ( _vst_ ), normalized counts matrix ( _counts_ ) , and DE table ( _prob_con_ )
```{r}
...
Calculate DE for all clusters (parallelized). DE tables obtained are then corrected for outliers sometimes found in output when a gene is only expressed a low sample in one condition - strategy used is to exclude all genes which have an absolute fold change of more than mean fold change +/- 40 x s.d. of the fold change.
```{r}
library(parallel)
...
a <- lapply(all_used_cell_types, function(i){
  ...
  volcano_plots[[i]] <<- 
    ...
    geom_jitter(aes(color = abs(log2FoldChange) > l2fc_thresh & padj < padj_thresh), size = 2.5, alpha = .8) + 
    ...
    geom_text_repel(max.overlaps =20, 
                    ...
                    data = filt_res_tbl %>% filter(abs(log2FoldChange) > l2fc_thresh & -log2(padj) > -log2(padj_thresh))) + 
    scale_color_manual(values = setNames(c("salmon", "steelblue"), c(TRUE, FALSE))) +
    ggtitle(i) +
    xlab("") + #Log2FC
    ylab("") + #-Log(padj)
    theme_classic() +
...
p1 <- all_comps_prob.con %>% filter(clust == sig) %>%
  ...
  theme(legend.position = "none", 
        ...
        strip.background = element_blank(),
        strip.text.x = element_text(size = 10, margin = margin(.2, 0, .1, 0, "cm")),
        strip.text.y = element_text(size = 10, margin = margin(.2, 0, .1, 0, "cm")),
        plot.title = element_text(hjust = 0.5, size = 14),
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank()) +
  xlab("") + ylab("Sum of gene-level z-scores")
...
p2 <- all_comp_prob.con_stats %>% filter(clust != sig) %>% ggplot() + 
  ...
  theme(legend.position = "none", 
        ...
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank()) + 
  xlab("-Log2(adj. p)") + ylab("Distribution (density)")
...
Load in genesets for enrichment analysis (obtained from MSigDb)
```{r}
genesets.dir <- "supp_data/genesets/"
GO_complete <- fgsea::gmtPathways(str_c(genesets.dir, "c5.go.v7.2.symbols.gmt"))
KEGG_complete <- fgsea::gmtPathways(str_c(genesets.dir, "c2.cp.kegg.v7.2.symbols.gmt"))
HPO_complete <- fgsea::gmtPathways(str_c(genesets.dir, "c5.hpo.v7.2.symbols.gmt"))
all_genes <- cluster_DE_results$`CD14+ Monocytes`$prob_con$gene
```
...
KEGG_subset <- list()
x <- lapply(names(KEGG_complete), function(i){
  KEGG_subset[[i]] <<- KEGG_complete[[i]][KEGG_complete[[i]] %in% all_genes]
})

HPO_subset <- list()
x <- lapply(names(HPO_complete), function(i){
  HPO_subset[[i]] <<- HPO_complete[[i]][HPO_complete[[i]] %in% all_genes]
})
```

Parallelized implementation of enrichment using custom-built genesetEnrichment function from custom package utility.functions
```{r}
gc()
...
mw_enrichment_tbls_hpo <- data.frame()
x <- mclapply(all_used_cell_types, FUN = function(i){
  rbind(genesetEnrichment(ranked_list = prepRankedGeneList(excludeSexChrGenes(all_DE[[i]]$prob_con, filt_mapping) %>% filter(!is.na(padj))), 
                                gene_set_list = HPO_subset, max_size_gene_set = 100,
                                hypothesis = "greater") %>% mutate(cluster = i, dir = "up"),
  genesetEnrichment(ranked_list = prepRankedGeneList(excludeSexChrGenes(all_DE[[i]]$prob_con, filt_mapping) %>% filter(!is.na(padj))), 
                                gene_set_list = HPO_subset, max_size_gene_set = 100,
                                hypothesis = "less") %>% mutate(cluster = i, dir = "down"))
...
mw_enrichment_tbls_hpo <- Reduce(rbind, x)

gc()

mw_enrichment_tbls_kegg <- data.frame()
x <- mclapply(all_used_cell_types, FUN = function(i){
  rbind(genesetEnrichment(ranked_list = prepRankedGeneList(excludeSexChrGenes(all_DE[[i]]$prob_con, filt_mapping) %>% filter(!is.na(padj))), 
                                gene_set_list = KEGG_subset, max_size_gene_set = 100,
                                hypothesis = "greater") %>% mutate(cluster = i, dir = "up"),
  genesetEnrichment(ranked_list = prepRankedGeneList(excludeSexChrGenes(all_DE[[i]]$prob_con, filt_mapping) %>% filter(!is.na(padj))), 
                                gene_set_list = KEGG_subset, max_size_gene_set = 100,
                                hypothesis = "less") %>% mutate(cluster = i, dir = "down"))
...
mw_enrichment_tbls_kegg <- Reduce(rbind, x)
...
png(filename = str_c(output.dir, "heatmap_singleton_enrich.png"), height = 8, width = 9, units = "in", res = 1200)
...
Function to calculate DE for male samples (male probands vs male controls) (function adapted from _analyze_DE_shrunk_prob_con_ above)
```{r}
analyze_DE_shrunk_male <- function(seurat_obj, cluster, slot = "counts", assay = "RNA"){
  seurat_obj <- subset(seurat_obj, ident = cluster)
  expr <- AggregateExpression(seurat_obj, assays = assay, slot = slot, group.by = c("smpl"))
  counts <- round(x = expr$RNA, digits = 0)
  meta.data <- seurat_obj@meta.data
...
male_DE <- list()
x <- mclapply(X = used_cell_types, FUN = function(i){
  male_DE[[i]] <<- analyze_DE_shrunk_male(seurat_obj = male_data, cluster = i)
}, mc.cores = 8)
...
male_DE_mono <- list()
x <- mclapply(X = used_cell_types_mono, FUN = function(i){
  male_DE_mono[[i]] <<- analyze_DE_shrunk_male(seurat_obj = male_mono, cluster = i)
}, mc.cores = 8)
...
p1 <- all_comps %>% filter(clust == sig) %>%
 ...
 scale_fill_manual(values = c("steelblue", "springgreen4", "salmon"), breaks = c("Control", "Carrier", "Proband")) +
  scale_color_manual(values = c("steelblue", "springgreen4", "salmon"), breaks = c("Control", "Carrier", "Proband")) +
  theme(legend.position = "none", 
        axis.title.x = element_text(size = 12),
        axis.title.y = element_text(size = 12),
...
This ranking can be converted by means of an AUROC using the following formula - 
...
The AUROC is evaluating the ability of each individual gene to rank disease samples above control samples, however since we have only 10 carrier and 7 female control samples, it is important to evaluate the null distribution of AUROCs by random permutation of the ordering of samples (since only a small number of AUROC values are possible). This task also lends itself rather well to multi-core processing as each permutation can be calculated individually. 
```{r}
...
res.pred.carrier %>% filter(mode == "test") %>% mutate(clust = factor(clust, levels = cell_order)) %>%
  ...
  #geom_hline(yintercept = 0.7285714, linetype = "dotted", color = "grey40") + 
...
rocs %>% 
  ...
  theme(legend.position = "none", 
        axis.title.x = element_text(size = 14),
        axis.title.y = element_text(size = 14),
        axis.text.x = element_text(size = 14, angle = 90, hjust = 1.0, vjust = 0.5),
        axis.text.y = element_text(size = 14),
        strip.text = element_text(size = 14),
        strip.background = element_blank())
```
...
```{r}
tmp <- res.pred.carrier %>% filter(sig %in% unique(res.pred.carrier$clust)) %>% pivot_wider(id_cols = sig, names_from = clust, values_from = roc) %>% column_to_rownames("sig")
...
Add human reference co-expression network to list of other networks
```{r}
...
Evaluate co-expression conservation across species using 1-1 orthologs of cluster-specific proband-derived disease signatures
with EGAD
```{r}
...
merge(final_res %>% 
  ...
  theme(legend.position = "none", 
        ...
        plot.title = element_text(hjust = 0.5, size = 14),
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank()) + facet_wrap(~cell_type) + stat_cor(method = "pearson", label.y.npc="bottom", label.x.npc = "left")
```
